    ...
    void process_request(http::request<request_body_t, http::basic_fields<alloc_t>> const& req)
    {
        ...
        case http::verb::get: {
            ...
            if (target.find(":") != std::string::npos) {
                std::string segment;
                bool path_extracted = false;
                int t;
                while (std::getline(targetstream, segment, ':')) {
                    if (path_extracted) {
                        t = segment.find("=");
                        if (t != std::string::npos) {
                            fields[segment.substr(0, t)] = segment.substr(t + 1, segment.size());
                        }
                    } else {
                        path = segment;
                        path_extracted = true;
        ...
        "ProjectDB", 0, NULL, 0) == NULL)
                ...
                char query[300];
                sprintf(query, "select * from FaceT order by id desc limit %d", stoi(fields["n"]));
                if (mysql_query(con, query)) {
                    ...
                    std::ofstream Data ("DataLog.txt");
                    Data << "  id             Faces             Date & Time                    \n";
                    while ((row = mysql_fetch_row(result)))
                    {
                        Data << " ";
                        int fieldlen[3] = {15, 15, 30};
                        for(int i = 0; i < num_fields; i++)
                        {   
                            char Mem[fieldlen[i]];
                            int Mflag = 0;
                            for(int m = 0; m < fieldlen[i]; m++){
                                if((row[i])[m] != '\0') {
                                    Mem[m] = (row[i])[m];
                                }
                                else {
                    ...
                    Data.close();
                    send_file("/DataLog.txt");
                }


            } else if (path == "/AudioDB") {
                if (fields.count("n") > 0)
                    std::cout << fields["n"] << std::endl;
        ...
        "ProjectDB", 0, NULL, 0) == NULL)
                ...
                char query[300];
                sprintf(query, "select * from AudioT order by id desc limit %d", stoi(fields["n"]));
                if (mysql_query(con, query)) {
                    finish_with_error(con);
                } else {
                    ...
                    std::ofstream Data ("DataLogAudio.txt");
                    Data << "  id           AudioDB           Date & Time                    \n";
                    while ((row = mysql_fetch_row(result)))
                    {
                        Data << " ";
                        int fieldlen[3] = {15, 15, 30};
